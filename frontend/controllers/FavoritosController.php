...
class FavoritosController extends Controller
{
    ...
    {
        // 1. Inicializa a query como algo vazio.
        $query = \common\models\Favoritos::find()->where('0=1');
        
        // 2. Verifica se o utilizador está autenticado.
        if (!Yii::$app->user->isGuest) {
            
            // Se estiver autenticado, obtém o utilizador e a query real.
            $idUser = Yii::$app->user->id;
            $user = Utilizadorperfil::findOne($idUser);

            // Verifica se o perfil do utilizador existe (deve existir se estiver logado)
            if ($user) {
                $query = $user->getFavoritos();
            }
        }

        // 3. Cria o DataProvider com a query (vazia ou com resultados)
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            /*
            ... (Tuas configurações de paginação e sort)
            */
        ]);
